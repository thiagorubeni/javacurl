...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		option = Option.builder("m")
				.longOpt("method")
				.hasArg()
				.desc("Method. Default is " + HttpGet.METHOD_NAME)
				.build();
		options.addOption(option);
		option = Option.builder("c")
				.longOpt("content")
				.hasArg()
				.desc("Content to send; overwrites \"f\"")
				.build();
		options.addOption(option);
		option = Option.builder("f")
				.longOpt("file")
				.hasArg()
				.desc("File with content to send")
				.build();
		options.addOption(option);
		option = Option.builder("H")
				.hasArgs()
				.desc("Header name-value pair parameters (\"-Hname=value\")")
				.build();
		options.addOption(option);
		option = Option.builder("s")
				.longOpt("status")
				.desc("Print HTTP status code")
				.build();
		options.addOption(option);
		option = Option.builder("keystore")
				.hasArg()
				.desc("Key store path")
				.build();
		options.addOption(option);
		option = Option.builder("keystoretype")
				.hasArg()
				.desc("Key store type; default is " + KeyStore.getDefaultType())
				.build();
		options.addOption(option);
		option = Option.builder("keystorepass")
				.hasArg()
				.desc("Key store password")
				.build();
		options.addOption(option);
		option = Option.builder("truststore")
				.hasArg()
				.desc("Trust store path")
				.build();
		options.addOption(option);
		option = Option.builder("truststoretype")
				.hasArg()
				.desc("Trust store type; default is " + KeyStore.getDefaultType())
				.build();
		options.addOption(option);
		option = Option.builder("truststorepass")
				.hasArg()
		...
		try {
			...
			String method = cmd.hasOption("m") ?
					cmd.getOptionValue("m").toUpperCase() :
					"GET";
			Properties header = cmd.getOptionProperties("H");
			String file = cmd.hasOption("f") ?
					cmd.getOptionValue("f") :
					null;
			String content = cmd.hasOption("c") ?
					cmd.getOptionValue("c") :
					null;
			String keyStore = cmd.hasOption("keystore") ?
					cmd.getOptionValue("keystore") :
					null;
			String keyStoreType = cmd.hasOption("keystoretype") ?
					cmd.getOptionValue("keystoretype") :
					KeyStore.getDefaultType();
			char[] keyStorePass = cmd.hasOption("keystorepass") ?
					cmd.getOptionValue("keystorepass").toCharArray() :
					null;
			...
			String trustStore = cmd.hasOption("truststore") ?
					cmd.getOptionValue("truststore") :
					null;
			String trustStoreType = cmd.hasOption("truststoretype") ?
					cmd.getOptionValue("truststoretype") :
					KeyStore.getDefaultType();
			char[] trustStorePass = cmd.hasOption("truststorepass") ?
					cmd.getOptionValue("truststorepass").toCharArray() :
					null;
			...
			// Default content type; will be overwritten if corresponding header is set
			...
			if(!httpRequest.getURI().getScheme().equalsIgnoreCase("https")) {
				...
				if(trustStore != null) {
					...
					sslContextBuilder = sslContextBuilder.loadTrustMaterial(ks,
